...
_gml = logging.getLogger(__name__)
_gml.addHandler(logging.NullHandler())
...
class GribMessage(utils.AbstractDictionary, GribAbstractItem):
    def __init__(self, message_id, grib_file):
        ...
        _gml.info(f'GribMessage init id={self.get_id()}')
    ...
    def __del__(self):
        if self._id is not None:
            _gml.info(f'releasing GribMessage id={self.get_id()}')
    ...
    def __getitem__(self, key):
        ...
        if ecc.codes_is_missing(self.get_id(), key):
            _gml.warning(f'key={key} is has value=MISSING in the GRIB message: {str(self)}')
    ...
    def get_value_at(self, lat, lon):
        ...
        for same_latitude_points in points:
            ...
            for point in same_latitude_points:
                pi.InterpolationNode(label=pi.Longitude(point.lon), value=self.get_value_by_index(point.index), parent=lat_node)
        return root_node.interpolate((lat, lon))
...
class GribFile(GribAbstractItem):
    ...
    def __del__(self):
        if self._file is not None and not self._file.closed:
            _gml.info(f'releasing GribFile {str(self)}')
            self.get_file().close()
...
class GribFileIndexedBy(utils.AbstractDictionary, GribAbstractItem):
    ...
    def __del__(self):
        if self._id is not None:
            _gml.info(f'releasing GribFileIndexedBy id={self.get_id()}, filename={self._filename}')
...
class GribFileIndexedByWithCache(utils.AbstractCacheDictionary, GribFileIndexedBy):
    pass
...
class GribFileUniquelyIndexedByWithCache(utils.AbstractCacheDictionary, GribFileUniquelyIndexedBy):
    pass
